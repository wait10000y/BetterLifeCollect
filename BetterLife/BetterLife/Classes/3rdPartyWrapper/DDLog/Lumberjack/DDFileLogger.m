...
//  NSString *appId = [infoDictionary objectForKey:@"CFBundleName"];
//  
  NSString *fileName = [NSString stringWithFormat:@"%s",logMessage->file];
  NSString *tempName = [[fileName componentsSeparatedByString:@"/"] lastObject];
  if (tempName) {
    fileName = tempName;
  }
//  NSRange range = [fileName rangeOfString:appId options:NSBackwardsSearch];
//  if (range.length>0) {fileName = [fileName substringFromIndex:range.location];}
