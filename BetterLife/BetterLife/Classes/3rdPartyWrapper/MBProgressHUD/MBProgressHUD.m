...
static MBProgressHUD *mCoverView;
+(MBProgressHUD *)createCoverView:(UIView *)tView withType:(MBProgressHUDMsgType)theType withDelegate:(id)theDelegate animated:(BOOL)theAnimated
{
  mCoverView = [[MBProgressHUD alloc] initWithView:tView];
  UIView *customView;
  if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
    mCoverView.minSize = CGSizeMake(260, 195);
    if (theType == MBProgressHUDMsg_loading) {
      customView = [MBSpinningCircle circleWithSize:NSSpinningCircleSizeLarge color:nil];
    }else if (theType == MBProgressHUDMsg_ok) {
      customView = [MBCheckMarkView checkMarkWithSize:MBCheckmarkSizeLarge color:nil];
    }else if (theType == MBProgressHUDMsg_warn) {
      customView = [MBCheckMarkView warnViewWithSize:MBCheckmarkSizeLarge color:nil];
    }else if (theType == MBProgressHUDMsg_error) {
      customView = [MBErrorMarkView errorMarkWithSize:MBErrormarkSizeLarge color:nil];
    }else if (theType == MBProgressHUDMsg_noStatus){
      customView = [UIView new];
    }
  }else{
    mCoverView.minSize = CGSizeMake(120, 90);
    if (theType == MBProgressHUDMsg_loading) {
      customView = [MBSpinningCircle circleWithSize:NSSpinningCircleSizeDefault color:nil];
    }else if (theType == MBProgressHUDMsg_ok) {
      customView = [MBCheckMarkView checkMarkWithSize:MBCheckmarkSizeSmall color:nil];
    }else if (theType == MBProgressHUDMsg_warn) {
      customView = [MBCheckMarkView warnViewWithSize:MBCheckmarkSizeSmall color:nil];
    }else if (theType == MBProgressHUDMsg_error) {
      customView = [MBErrorMarkView errorMarkWithSize:MBErrormarkSizeSmall color:nil];
    }else if (theType == MBProgressHUDMsg_noStatus){
      customView = [UIView new];
    }
  }
  mCoverView.removeFromSuperViewOnHide = YES;
  mCoverView.dimBackground = YES;
  mCoverView.delegate = theDelegate;
  
  if (customView) {
    mCoverView.customView = customView;
    mCoverView.mode = MBProgressHUDModeCustomView;
  }else{
    mCoverView.customView = nil;
    mCoverView.mode = MBProgressHUDModeIndeterminate;
  }
  
  mCoverView.tag = theType;
  [tView addSubview:mCoverView];
    [mCoverView show:theAnimated];
  return mCoverView;
}
+(MBProgressHUD *)showGlobViewWithType:(MBProgressHUDMsgType)theType withDelegate:(id<MBProgressHUDDelegate>)theDelegate animated:(BOOL)theAnimated
{
  UIView *tView = nil;
  if (!tView) {
    tView = [UIApplication sharedApplication].windows[0];
  }
  if (mCoverView) { [mCoverView hide:NO]; }
  
  if ([NSThread currentThread].isMainThread) {
    [MBProgressHUD createCoverView:tView withType:theType withDelegate:theDelegate animated:theAnimated];
  }else{
...
+(void)updateGlobViewWithType:(MBProgressHUDMsgType)theType withTips:(NSString *)theTips withMessage:(NSString *)theMsg
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (!mCoverView) {
      [MBProgressHUD showGlobViewWithType:MBProgressHUDMsg_loading withDelegate:nil animated:NO];
    }
    mCoverView.labelText = theTips;
    mCoverView.detailsLabelText = theMsg;
    if (theType !=mCoverView.tag) {
      mCoverView.tag = theType;
      ...
      if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
        if (theType == MBProgressHUDMsg_loading) {
          ...
          customView = [UIView new];
        }else if (theType == MBProgressHUDMsg_systomLoading){
          customView = nil;
        }
      }else{
        if (theType == MBProgressHUDMsg_loading) {
          ...
          customView = [MBCheckMarkView checkMarkWithSize:MBCheckmarkSizeMedium color:nil];
        }else if (theType == MBProgressHUDMsg_warn) {
          customView = [MBCheckMarkView warnViewWithSize:MBCheckmarkSizeMedium color:nil];
        }else if (theType == MBProgressHUDMsg_error) {
          customView = [MBErrorMarkView errorMarkWithSize:MBErrormarkSizeMedium color:nil];
        }else if (theType == MBProgressHUDMsg_noStatus){
          customView = [UIView new];
        }else if (theType == MBProgressHUDMsg_systomLoading){
          customView = nil;
      ...
      if (customView) {
        ...
        if (theType != MBProgressHUDMsg_noChange) {
          mCoverView.customView = nil;
          mCoverView.mode = MBProgressHUDModeIndeterminate;
        }
      }
...
+(void)hideGlobView:(BOOL)theAnimated withDelay:(NSTimeInterval)theTime
{
  dispatch_async(dispatch_get_main_queue(), ^{
    if (mCoverView) {
      if (theTime < 0.005) {
        [mCoverView hide:theAnimated];
      }else{
        [mCoverView hide:theAnimated afterDelay:theTime];
      }
      mCoverView = nil;
    }
  });
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
      ...
      float maxLWidth = frame.size.width - 4 * margin;
//        CGSize dims = [self.labelText sizeWithFont:self.labelFont forWidth:maxLWidth lineBreakMode:NSLineBreakByWordWrapping];
      CGSize dims = [self.labelText sizeWithFont:self.labelFont constrainedToSize:CGSizeMake(maxLWidth, 100)];
        // Compute label dimensions based on font metrics if size is larger than max then clip the label width
        float lHeight = dims.height*2;
        float lWidth = maxLWidth;
        if (dims.width <= maxLWidth) {
            lWidth = dims.width;
        }
...
- (void)show:(BOOL)animated {
	...
	[self removeTarget:self action:@selector(hideViewNow) forControlEvents:UIControlEventTouchUpInside];
...
- (void)hide:(BOOL)animated afterDelay:(NSTimeInterval)delay {
  [self addTarget:self action:@selector(hideViewNow) forControlEvents:UIControlEventTouchUpInside];
